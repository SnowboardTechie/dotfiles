...
GNARBOX_PROFILE=false
if [[ "$PLATFORM" == "linux" && "$(hostname -s)" == "gnarbox" ]]; then
    GNARBOX_PROFILE=true
fi
...
if [[ "$GNARBOX_PROFILE" == true ]]; then
    RECONCILE_OPENCODE_SKILLS=0 "$REPO_ROOT/scripts/reconcile-agent-skills.sh" --apply
else
