...
8. **Opus 5.5 primary selection.** Claude implementation and revision passes select exact model `claude-opus-5-5`. Any other primary model requires the user's explicit confirmation and the wrapper's `--allow-non-opus` acknowledgement; overload never authorizes a silent downgrade. When the user requires literal model purity, load `coding-agent-model-purity`: `--model claude-opus-5-5` alone does not prove that internal helpers used only Opus 5.5.
...
Always use `--model claude-opus-5-5` for implementation and revision. Do not fall back or downgrade to Opus 5, Sonnet, Haiku, or another primary model after overload, throttling, or model unavailability. Stop and ask the user first. After explicit confirmation, an alternate-model invocation must also include `--allow-non-opus`.
...
11. **Treating overload as an implementation failure.** Claude may return HTTP 429/529 before doing work. The wrapper retries the same Opus 5.5 request once by default; if both attempts fail, preserve repository state and report the provider outage. Never downgrade models without explicit user confirmation.
