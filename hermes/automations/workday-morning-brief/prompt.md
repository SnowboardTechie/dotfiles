...
13. Use Apple Calendar as the calendar source, but completely exclude the calendar named Traci and every event from it. Convert times to America/Los_Angeles. Separate all-day items from timed commitments. For each relevant all-day item, state its actual event title or action; never replace it with a vague label such as all-day household reminder. Mention personal events only as scheduling constraints when they affect the day; do not expose unnecessary private detail.
